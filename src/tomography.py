...
z_list = np.array(constants.elevation)
p0_list = np.array(constants.mirror)
...
for i, (reflector, u, theta, T, P, L) in enumerate(zip(reflectors, wind_speed, plume_dir, temp, pressure, lvals)):
    ...
    reading_predicted[i] = util.line_average([source_x, source_y], p0_list[reflector], p1_list[reflector], z_list[reflector], samples, 1, h_source, theta, T,
                                  P, params, 'gaussian')
