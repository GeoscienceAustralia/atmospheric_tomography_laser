...
###########################################################################################################
def line_average(s_loc, p0, p1, height, n, Q, H, theta, T, P, dict, method):
    vec = rotate_los(s_loc, p0, p1, theta, n)
    line_samples = samples(vec[0], vec[1], height, Q, dict, method)
    if method != 'semi-gaussian':
        line_samples = [gm_3toppmv(ls, T, P, constants.molar_mass) for ls in line_samples]
...
###########################################################################################################
def line_integral(s_loc, p0, p1, height, n, Q, H, theta, T, P, dict, method):
    t = np.linspace(0, 1, n)
    vec = rotate_los(s_loc, p0, p1, theta, n)
    line_samples = samples(vec[0], vec[1], height, Q, dict, method)
    if method != 'semi-gaussian':
